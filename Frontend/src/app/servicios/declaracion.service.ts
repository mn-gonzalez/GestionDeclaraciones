...
export class DeclaracionService {
  ...
  */
  ...
  obtenerDatosGrafico(year: number){
    return this.http.get<{declaraciones_mensuales: {mes: number, total: number}[], nro_declaraciones: number, declaraciones_sin_revisar: number}>(env.api.concat("/declaraciones_mensuales/"+year))
    .pipe(
      map(result => {
        return result;
      })
